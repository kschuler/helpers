...
Here I have four data tables in my participants.db. Each data table corresponds to the data for particular experiment. You can access a specific table using the SQL engine: `{sql connection=mturk.data, output.var="mturk"}`. Here, `connection=mturk.data` connects you to the database you defined above and `output.var="mturk"` assigns the results of the SQL query to an R data frame. I select all fields from the `0155v1` data table.
...
And we also want to be able to exclude particular workers by their id number.  This comes up for me sometimes when workers participate in more than one of my experiments (even when I ask them not to do that).
...
I like to inspect the data at this point to see how many workers are now included in each condition.
